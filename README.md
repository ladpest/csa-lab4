...
| `ST rd, rs1` | `MEM[rd] <- rs1` | 4 |
| `JMP imm` | `PC <- imm` | 2 |
| `JZ rs1, imm` | если `rs1 == 0`, то `PC <- imm` | 2 |
| `CALL imm` | сохранить `PC`, перейти на `imm` | 5 |
| `CALLR rs1` | сохранить `PC`, перейти по адресу из `rs1` | 7 |
| `RET` | восстановить `PC` из стека возвратов | 5 |
| `HLT` | остановка процессора | 2 |

Полный цикл включает оптимизированный микротакт выборки (`IR <- MEM[PC]`, `PC <- PC + 1`) и микротакты исполнения выбранной микропрограммы. Ветвления не требуют отдельной логики сброса: после fetch `PC` содержит адрес следующей инструкции по умолчанию, а микрокоманда `JMP`/`JZ` при необходимости перезаписывает `PC` целевым адресом.
...
| `ST` | `9` | `st_ar_rd`, `st_dr_rs1`, `st_mem_dr` |
| `JMP` | `12` | `jmp_imm` |
| `JZ` | `13` | `jz_rs1_imm` |
| `CALL` | `14` | `call_ar_r15`, `call_dr_pc`, `call_store_ret`, `call_r15_inc_pc_imm` |
| `CALLR` | `18` | `callr_ar_r15`, `callr_dr_pc`, `callr_store_ret`, `callr_r15_inc`, `callr_dr_rs1`, `callr_pc_dr` |
| `RET` | `24` | `ret_r15_dec`, `ret_ar_r15`, `ret_dr_mem`, `ret_pc_dr` |
| `HLT` | `28` | `halt` |

В журнале модели для каждого микротакта выводятся `uPC`, значение `MIR` в hex, имя микрокоманды, активные управляющие поля и состояние регистров. По этому журналу можно проверить, что модель идёт именно по микрокомандам.
